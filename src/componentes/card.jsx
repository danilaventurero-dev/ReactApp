...
const card = ({ dispatch,items, jwt}) => {
  ...
  return (
    <div>
      <Card>
        <Link className='' to={`/articulo/${items.id}`} >
        <CardImg top width="100%" src={items.image_url} alt="Card image cap" />
        </Link>
        <CardBody>
          <CardTitle>{items.title}</CardTitle>
          <CardSubtitle>{items.description.substring(0, 20)}...</CardSubtitle>
          <CardText></CardText>
          {jwt ? (
            <div>
            <Button color="primary" size="sm" onClick={() => dispatch(findAction(jwt,items.id))} > Editar</Button>
